...
export default function AuditPage() {
  ...
  // Sorting state
  const [sortField, setSortField] = useState<'timestamp' | 'status' | 'action'>('timestamp');
  ...
  const logs = [
    {
      ...
      ip: "192.168.1.150",
      details: "{\"DeviceName\":\"S25 Ultra Mock\",\"TrustScore\":100,\"PartitionVerification\":\"PASSED\"}"
    },
    {
      id: "log-38491-ff12",
      ...
      ip: "10.0.12.80",
      details: "{\"WipedBlock\":\"/persistent\",\"ConsensusVerified\":true,\"PolicyApproved\":true}"
    },
    {
      id: "log-02849-aa92",
      ...
      ip: "172.16.89.12",
      details: "{\"Reason\":\"Knox Warranty Void blown 0x1\",\"Selinux\":\"Permissive\"}"
    },
    {
      id: "log-93810-bc56",
    ...
    },
    {
      ...
      device: "Standard S24 Dev Node",
      ip: "10.0.12.80",
      details: "{\"Reason\":\"Unauthorized user role. Action requires Admin-level credentials.\"}"
    }
  ];
  ...
  const failurePercentage = totalEventCount > 0 ? ((deniedLogs.length / totalEventCount) * 100).toFixed(1) : '0.0';
      ...
      <span>
        {parts.map((part, i) => 
          part.toLowerCase() === query.toLowerCase() ? (
            <mark key={i} className="bg-amber-400 text-black px-0.5 rounded font-bold">
              {part}
            </mark>
          ) : (
            part
  ...
  const handleExportCSV = () => {
    const headers = ["ID", "Timestamp (UTC)", "Operator", "Action", "Status", "Target Device", "IP Address", "Details JSON"];
    ...
    link.setAttribute("download", `acing_iu_compliance_audit_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[#22314D] pb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Secure write-once-read-many (WORM) audit ledger tracking Knox transitions and system operations.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-6 rounded-2xl flex flex-col justify-between space-y-4">
          <div className="space-y-2">
            ...
            <h3 className="text-lg font-bold text-white">CTIA 3.8.2 RF & Knox Security Status</h3>
            <p className="text-xs text-slate-400 leading-relaxed font-medium">
              Real-time audit telemetry tracking verified Knox 0x0 warranty, SELinux parameters, and Verizon hardware RF compliance thresholds.
            </p>
          </div>
            ...
            <div className="h-[200px] w-full flex items-center justify-center">
              {isMounted ? (
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={hourlyChartData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                    ...
                    <Line 
                      ...
                      stroke="#10B981" 
                      strokeWidth={2.5} 
                      dot={{ r: 3, stroke: '#10B981', strokeWidth: 1, fill: '#111827' }}
                      activeDot={{ r: 5 }} 
                    />
                    <Line 
                      type="monotone" 
                      ...
                      stroke="#EF4444" 
                      strokeWidth={2.5} 
                      dot={{ r: 3, stroke: '#EF4444', strokeWidth: 1, fill: '#111827' }}
                      activeDot={{ r: 5 }}
                    />
                  </LineChart>
                </ResponsiveContainer>
            ...
            <div className="relative space-y-4">
              ...
              <div className="grid grid-cols-5 sm:grid-cols-10 gap-2">
                {heatmapData.map((day, idx) => {
                  ...
                  if (day.total > 0 && day.total <= 10) {
                    cellBg = "bg-[#2F58CD]/20 border-[#2F58CD]/30 text-blue-300 hover:bg-[#2F58CD]/30";
                  } else if (day.total > 10 && day.total <= 20) {
                    cellBg = "bg-[#2F58CD]/45 border-[#2F58CD]/50 text-blue-100 hover:bg-[#2F58CD]/55";
                  } else if (day.total > 20 && day.total <= 30) {
                    cellBg = "bg-[#2F58CD]/70 border-[#2F58CD]/80 text-white hover:bg-[#2F58CD]/80";
                  } else if (day.total > 30) {
                    cellBg = "bg-[#2F58CD] border-white/25 text-white font-bold hover:brightness-110";
                  }
                  ...
                  return (
                    ...
                    >
                      <span className="font-extrabold text-[11px]">{day.day}</span>
                      <span className="text-[7.5px] opacity-70 uppercase tracking-tight">{day.month}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="p-3 bg-[#0B0F19] border border-[#22314D] rounded-xl flex items-center justify-between min-h-[58px] transition-all duration-300">
                {hoveredDay ? (
                  <>
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${hoveredDay.denied > 0 ? 'bg-red-950/40 text-red-400 border border-red-900/30' : 'bg-blue-950/40 text-blue-400 border border-blue-900/30'}`}>
                        <Calendar className="h-4 w-4" />
                      </div>
                      <div>
                        <span className="text-[9px] text-slate-400 font-extrabold uppercase block tracking-wider">
                          {hoveredDay.dayName}, {hoveredDay.month} {hoveredDay.day}, 2026
                        </span>
                        <span className="text-xs font-bold text-white block mt-0.5">
                          Total Event Audits: <strong className="text-blue-400">{hoveredDay.total}</strong>
                        </span>
                      </div>
                    </div>
                    <div className="flex gap-4 text-xs font-mono pr-2">
                      <div className="flex flex-col items-end">
                        <span className="text-[8px] text-slate-500 font-bold uppercase">Success</span>
                        <span className="text-emerald-400 font-bold text-xs">{hoveredDay.success}</span>
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="text-[8px] text-slate-500 font-bold uppercase">Denied</span>
                        <span className={`font-bold text-xs ${hoveredDay.denied > 0 ? 'text-red-500' : 'text-slate-500'}`}>
                          {hoveredDay.denied}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="text-xs text-slate-400 italic flex items-center gap-2 mx-auto font-medium">
                    ...
                    <span>Hover over any cell in the 30-day calendar to explore specific audit frequency and security event counts.</span>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-[#22314D]">
          {sortedLogs.length > 0 ? (
            sortedLogs.map((log) => (
              <div key={log.id} className="p-5 hover:bg-[#151D30]/30 transition-all duration-200 space-y-3">
                {/* Top metadata block */}
                <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-slate-400 font-medium">
                      Device: <strong className="text-slate-300 font-semibold">{highlightText(log.device, searchTerm)}</strong>
                    </span>
                  </div>
                ...
                <div className="p-3.5 bg-[#0B0F19]/90 rounded-xl border border-[#22314D] flex flex-col md:flex-row md:items-center justify-between gap-3 text-[11px] font-mono">
                  <div className="text-slate-400 truncate max-w-3xl">
                    Payload: <span className="text-[#10B981] font-semibold">{highlightText(log.details, searchTerm)}</span>
                  </div>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400 font-medium">
                  <span>Operator: <strong className="text-slate-300">{highlightText(log.user, searchTerm)}</strong></span>
                  <span className={`font-bold flex items-center gap-1.5 ${
                    log.status === 'SUCCESS' ? 'text-[#10B981]' : 'text-[#EF4444]'
                  }`}>
                    {log.status === 'SUCCESS' ? <CheckCircle className="h-3.5 w-3.5" /> : <ShieldAlert className="h-3.5 w-3.5" />}
            ...
            <div className="p-12 text-center text-slate-500 text-xs font-semibold">
              No matching security audit logs found. Try adjusting your query parameters.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="print-report-container">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
          <div>
            <strong>DOCUMENT ID:</strong> TX-COMP-S938U-2026-VZW<br />
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <strong>COMPLIANCE SCOPE:</strong> VRU3CXH2 BASELINE<br />
            <strong>CURRENT CORE INTEGRITY:</strong> <span className="print-badge" style={{ borderColor: deniedLogs.length > 0 ? 'red' : 'green', color: deniedLogs.length > 0 ? 'red' : 'green' }}>{deniedLogs.length > 0 ? 'WARNING_DEGRADED' : 'PASSED'}</span>
          </div>
        </div>

        {/* Dynamic PDF Summary Statistics Section */}
        <div className="print-section-title">Summary Statistics & Security Posture</div>
        <div className="print-grid" style={{ marginBottom: '20px', border: '1px solid #333333', padding: '12px', backgroundColor: '#f9f9f9' }}>
          <div>
            <strong>Total Events Evaluated:</strong> {totalEventCount}<br />
            <strong>Successful Operations:</strong> {successLogs.length}<br />
            <strong>Compliance Failure Rate:</strong> <span style={{ color: deniedLogs.length > 0 ? 'red' : 'black', fontWeight: 'bold' }}>{failurePercentage}% ({deniedLogs.length} failed event{deniedLogs.length === 1 ? '' : 's'})</span>
          </div>
          <div>
            <strong>Critical Security Warnings ({deniedLogs.length}):</strong>
            {deniedLogs.length > 0 ? (
              <ul style={{ margin: '5px 0 0 15px', padding: 0, fontSize: '8.5pt', listStyleType: 'square' }}>
              ...
              <span style={{ color: 'green', marginLeft: '5px', fontWeight: 'bold' }}>None. No system violations detected during this audit window.</span>
            )}
          </div>
        </div>
        ...
        <div className="print-grid" style={{ marginBottom: '15px' }}>
          <div>
            <strong>Knox Hardware root:</strong> 0x0 Warranty Intact (Passed)<br />
            <strong>SELinux state:</strong> Enforcing (Passed)<br />
            <strong>TIMA Kernel Guard:</strong> Active (Passed)<br />
            <strong>Bootloader status:</strong> Locked (Passed)
          </div>
          <div>
            <strong>Total Radiated Power (TRP):</strong> 23.40 dBm (Target &gt;= 23.0 dBm) [PASSED]<br />
            <strong>Isotropic Sensitivity (TIS):</strong> -92.15 dBm (Target &lt;= -90.0 dBm) [PASSED]<br />
            <strong>super.img Baseline SHA:</strong> Approved Matches Perfect
          </div>
        </div>

        <div className="print-section-title">Verified Active logging Ledger</div>
          ...
          <tbody>
            {sortedLogs.map(log => (
              <tr key={log.id}>
                ...
                <td>
                  <span className="print-badge" style={{ borderColor: log.status === 'SUCCESS' ? 'green' : 'red', color: log.status === 'SUCCESS' ? 'green' : 'red' }}>
                    {log.status}
                  </span>
                </td>
                <td style={{ fontFamily: 'monospace', fontSize: '8pt', wordBreak: 'break-all' }}>{log.details}</td>
              </tr>
            ))}
          </tbody>
        </table>

        <div style={{ marginTop: '50px', borderTop: '1px solid #999', paddingTop: '10px', fontSize: '9pt', display: 'flex', justifyContent: 'space-between' }}>
          <div>
          ...
          <div className="bg-[#111827] border border-[#22314D] rounded-3xl max-w-4xl w-full p-6 space-y-6 shadow-2xl relative animate-slideIn text-left">
            <div className="flex items-center justify-between border-b border-[#22314D] pb-4">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="text-base font-bold text-white">Export Filtered CSV Preview</h3>
              </div>
              <button 
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              Reviewing the filtered active compliance audit logs below prior to exporting as a formatted CSV spreadsheet:
            </p>
            ...
            <div className="overflow-x-auto border border-[#22314D] rounded-xl max-h-[300px] scrollbar-thin">
              <table className="w-full text-xs text-slate-300 border-collapse">
                ...
                <tbody className="divide-y divide-[#22314D]">
                  {sortedLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-[#151D30]/30 transition-colors">
                      <td className="p-3 font-mono text-[9px] text-slate-500">{log.id}</td>
                      <td className="p-3 font-mono text-[10px] text-slate-400">{log.timestamp}</td>
                      <td className="p-3 font-semibold text-slate-400">{log.user}</td>
                      <td className="p-3 font-bold text-white">{log.action}</td>
                      <td className="p-3">
                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold ${
                          log.status === 'SUCCESS' ? 'bg-[#10B981]/20 text-[#10B981] border border-[#10B981]/30' : 'bg-red-500/20 text-red-500 border border-red-500/30'
                        }`}>
                          {log.status}
                        </span>
                      </td>
            ...
            <div className="flex justify-between items-center border-t border-[#22314D] pt-4">
              <span className="text-[10px] text-slate-500 font-bold uppercase">
                Ready to generate: {sortedLogs.length} record{sortedLogs.length === 1 ? '' : 's'}
              </span>
