...
const initialBuildLogsHistory = [
  ...
  },
  {
    ...
    logs: [
      ...
      "❌ Compilation failed during third stage run."
    ]
  },
  {
    id: "Build #4",
...
const getMultiFileZipBlob = (files: { filename: string, content: string }[]) => {
  ...
  // Combine everything
  const finalBlobParts: Uint8Array[] = [...parts, ...cdEntries, eocdBytes];
  return new Blob(finalBlobParts, { type: 'application/zip' });
};
...
const CustomBuildTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="bg-[#0B0F19] border-2 border-[#22314D] p-3.5 rounded-xl shadow-2xl text-[10px] font-mono space-y-2 text-left max-w-[220px] backdrop-blur-md">
        <div className="font-extrabold text-white text-[11px] border-b border-[#22314D] pb-1 uppercase tracking-wider flex items-center justify-between">
          <span>{label}</span>
          <span className="text-indigo-400 font-extrabold text-[9px] bg-indigo-950/40 border border-indigo-500/20 px-1.5 py-0.5 rounded">STATS</span>
        </div>
        <div className="space-y-1.5 pt-1">
          <div className="flex justify-between items-center gap-4">
            <span className="text-slate-400">ISO Size:</span>
            <span className="font-bold text-blue-400">{data.size || 'N/A'} MB</span>
          </div>
          <div className="flex justify-between items-center gap-4">
            <span className="text-slate-400">Duration:</span>
            <span className="font-bold text-amber-400">{data.duration ? `${data.duration} sec` : 'N/A'}</span>
          </div>
          <div className="flex justify-between items-center gap-4">
            <span className="text-slate-400">Compression Ratio:</span>
            <span className="font-bold text-emerald-400">{data.ratio ? `${data.ratio}:1` : 'N/A'}</span>
          </div>
          {data.predictedDuration !== undefined && data.predictedDuration !== null && (
            <div className="flex justify-between items-center gap-4 border-t border-[#22314D]/50 pt-1.5 mt-1.5">
              ...
              <span className="font-bold text-pink-400 animate-pulse">{data.predictedDuration} sec</span>
            </div>
          )}
        </div>
     function DoughnutChart({ successCount, failureCount }: { successCount: number, failureCount: number }) {
  ...
  const successStrokeDashoffset = circumference - (successRate / 100) * circumference;
    ...
    <div className="flex flex-col items-center justify-center space-y-3 p-4 bg-[#0B0F19]/40 rounded-2xl border border-[#22314D]/40 w-full h-full min-h-[180px]">
      ...
      <div className="relative w-28 h-28 flex items-center justify-center">
        ...
        <div className="absolute flex flex-col items-center justify-center text-center">
          <span className="text-xl font-black text-white leading-none">{successRate}%</span>
          <span className="text-[8px] font-extrabold text-slate-400 uppercase tracking-wider mt-0.5">Success</span>
        </div>
      </div>
...
function FirmwareBuildComparer({ successfulBuilds }: { successfulBuilds: any[] }) {
  ...
  const getDiffBadge = (diff: number, lowerIsBetter: boolean, unit: string) => {
    if (diff === 0) return <span className="text-slate-500 font-bold font-mono">0 {unit} (No Change)</span>;
    const sign = diff > 0 ? '+' : '';
    const isGood = lowerIsBetter ? diff < 0 : diff > 0;
    return (
      <span className={`font-extrabold font-mono text-[9px] px-2 py-0.5 rounded-md ${
        isGood ? 'bg-emerald-950/55 text-emerald-400 border border-emerald-900/30' : 'bg-red-950/55 text-red-400 border border-red-900/30'
      }`}>
        {sign}{diff} {unit} ({isGood ? 'Optimized' : 'Slower/Larger'})
      </span>
    );
  };
  ...
  return (
    <div className="glass-card p-6 rounded-2xl border border-[#22314D] space-y-5 bg-[#070B14]">
      <div className="flex items-center gap-2 border-b border-[#22314D] pb-3">
        ...
        <h3 className="text-xs font-extrabold text-white uppercase tracking-wider">Firmware Build Comparer (Side-by-Side)</h3>
      </div>

      <div className="grid grid-cols-2 gap-4">
        {/* Build A Selector */}
        <div className="space-y-1">
          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Baseline Build (A)</label>
          <select
            value={buildAId}
          ...
          >
            {successfulBuilds.map(b => (
              <option key={b.id} value={b.id}>{b.id} {b.status === 'failed' ? '(Failed)' : `(${b.size} MB)`}</option>
            ))}
          </select>
        </div>
        ...
        {/* Build B Selector */}
        <div className="space-y-1">
          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Comparison Build (B)</label>
          <select
            value={buildBId}
      ...
      <div className="space-y-4 pt-1">
        ...
        <div className="p-4 rounded-xl border border-[#22314D]/40 bg-[#0B0F19]/50 space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Partition Payload Size</span>
            {buildA.status !== 'failed' && buildB.status !== 'failed' && getDiffBadge(sizeDiff, true, 'MB')}
          </div>
          <div className="grid grid-cols-2 gap-4 text-center">
            <div className="p-2 bg-[#020617] rounded-lg border border-[#22314D]/30">
              <span className="block text-[10px] text-slate-500 font-bold uppercase">{buildA.id}</span>
              <span className="text-sm font-black text-white">{buildA.status === 'failed' ? 'N/A (Failed)' : `${buildA.size} MB`}</span>
            </div>
            <div className="p-2 bg-[#020617] rounded-lg border border-[#22314D]/30">
              <span className="block text-[10px] text-slate-500 font-bold uppercase">{buildB.id}</span>
              <span className="text-sm font-black text-white">{buildB.status === 'failed' ? 'N/A (Failed)' : `${buildB.size} MB`}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border border-[#22314D]/40 bg-[#0B0F19]/50 space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">SquashFS Compression Ratio</span>
            {buildA.status !== 'failed' && buildB.status !== 'failed' && getDiffBadge(ratioDiff, false, ':1')}
          </div>
          <div className="grid grid-cols-2 gap-4 text-center">
            <div className="p-2 bg-[#020617] rounded-lg border border-[#22314D]/30">
              <span className="block text-[10px] text-slate-500 font-bold uppercase">{buildA.id}</span>
              <span className="text-sm font-black text-amber-400">{buildA.status === 'failed' ? 'N/A (Failed)' : `${buildA.ratio}:1`}</span>
            </div>
            <div className="p-2 bg-[#020617] rounded-lg border border-[#22314D]/30">
              <span className="block text-[10px] text-slate-500 font-bold uppercase">{buildB.id}</span>
              <span className="text-sm font-black text-amber-400">{buildB.status === 'failed' ? 'N/A (Failed)' : `${buildB.ratio}:1`}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border border-[#22314D]/40 bg-[#0B0F19]/50 space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Compilation Duration</span>
            {buildA.status !== 'failed' && buildB.status !== 'failed' && getDiffBadge(durationDiff, true, 'sec')}
          </div>
          <div className="grid grid-cols-2 gap-4 text-center">
            <div className="p-2 bg-[#020617] rounded-lg border border-[#22314D]/30">
              <span className="block text-[10px] text-slate-500 font-bold uppercase">{buildA.id}</span>
              <span className="text-sm font-black text-purple-400">{buildA.duration} sec</span>
            </div>
            <div className="p-2 bg-[#020617] rounded-lg border border-[#22314D]/30">
              <span className="block text-[10px] text-slate-500 font-bold uppercase">{buildB.id}</span>
              <span className="text-sm font-black text-purple-400">{buildB.duration} sec</span>
            </div>
          </div>
        </div>
      </div>
...
function BuildHistoryChart({ successfulBuilds, onExportHistory }: { successfulBuilds: any[], onExportHistory: () => void }) {
  ...
  if (n > 1) {
    ...
    const sumXY = ratioValues.reduce((sum, r, i) => sum + r * durationValues[i], 0);
  ...
  const successCount = successfulBuilds.filter(b => b.status !== 'failed').length;
  const failureCount = successfulBuilds.filter(b => b.status === 'failed').length;
    ...
    <div className="glass-card p-5 rounded-2xl border border-[#22314D] space-y-4 bg-[#070B14]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#22314D] pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-extrabold text-white uppercase tracking-wider">Build History & Performance</h3>
        </div>
        <button
        ...
        <div className="lg:col-span-2 h-44 text-[10px]">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 10, right: -10, left: -25, bottom: 0 }}>
              ...
              <Bar yAxisId="left" dataKey="size" name="ISO Size (MB)" fill="#3B82F6" radius={[4, 4, 0, 0]} opacity={0.8} />
              <Line yAxisId="right" type="monotone" dataKey="duration" name="Duration (sec)" stroke="#F59E0B" strokeWidth={2} />
              <Line yAxisId="right" type="monotone" dataKey="ratio" name="Ratio (x:1)" stroke="#10B981" strokeWidth={2} />
              <Line yAxisId="right" type="monotone" dataKey="predictedDuration" name="Predicted Build Time (sec)" stroke="#EC4899" strokeWidth={2} strokeDasharray="5 5" />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
        <div className="flex items-center justify-center">
          <DoughnutChart successCount={successCount} failureCount={failureCount} />
        </div>
      </div>
    </div>
  );
...
const partitionDetails = {
  'system.img': {
    ...
    subPartitions: [
      { name: 'priv-app/SecSetupWizard', files: 34, permission: 'rwxr-xr-x (0755)', context: 'u:object_r:system_file:s0', rebrandingTarget: 'Acing Setup Wizard', details: 'Contains setup greeting, language, and network setup components. Configured for Acing welcome flow.' },
      { name: 'priv-app/SystemUI', files: 145, permission: 'rwxr-xr-x (0755)', context: 'u:object_r:system_file:s0', rebrandingTarget: 'Acing System UI', details: 'Manages status bar, lock screen, and quick toggles. Custom Acing branding applied.' },
      { name: 'media/bootsamsung.qmg', files: 1, permission: 'rw-r--r-- (0644)', context: 'u:object_r:system_file:s0', rebrandingTarget: 'bootsamsung.qmg', details: 'Samsung proprietary boot animation QMG format. Modified with Acing logo anim sequence.' },
      { name: 'media/bootsamsungloop.qmg', files: 1, permission: 'rw-r--r-- (0644)', context: 'u:object_r:system_file:s0', rebrandingTarget: 'bootsamsungloop.qmg', details: 'Primary looping boot animation during system start. Rebranded with high-tech logo.' },
      { name: 'framework/framework-res.apk', files: 82, permission: 'rw-r--r-- (0644)', context: 'u:object_r:system_file:s0', rebrandingTarget: 'framework-res.apk', details: 'Core system resources. Relabeled baseline One UI string descriptors to Acing IU.' }
    ]
  },
  'product.img': {
    fileCount: 8432,
    ...
    subPartitions: [
      { name: 'app/AcingWallet', files: 12, permission: 'rw-r--r-- (0644)', context: 'u:object_r:product_file:s0', rebrandingTarget: 'Acing Wallet dApp', details: 'Decentralized private key management app integrated with Knox trust chains.' },
      { name: 'overlay/CarrierConfigOverlay', files: 4, permission: 'rw-r--r-- (0644)', context: 'u:object_r:product_file:s0', rebrandingTarget: 'Verizon Custom Bypass', details: 'XML configurations adjusting mobile registration and overriding default system blocks.' }
    ]
  },
  'vendor.img': {
    fileCount: 4120,
    ...
    subPartitions: [
      { name: 'bin/hw/android.hardware.biometrics.fingerprint@2.1-service', files: 1, permission: 'rwxr-xr-x (0755)', context: 'u:object_r:hal_fingerprint_default_exec:s0', rebrandingTarget: 'Ultrasonic Biometrics Service', details: 'Handles low-level Snapdragon ultrasonic fingerprint biometric matching for multi-action triggers.' },
      { name: 'etc/init/android.hardware.biometrics.fingerprint@2.1-service.rc', files: 1, permission: 'rw-r--r-- (0644)', context: 'u:object_r:vendor_configs_file:s0', rebrandingTarget: 'Fingerprint RC Daemon', details: 'Init script registering bioservice daemon inside kernel workspace at boot.' }
    ]
  },
  'odm.img': {
    fileCount: 1248,
    permissionsStatus: '100% Secure',
    subPartitions: [
      { name: 'etc/vintf/manifest_odm.xml', files: 1, permission: 'rw-r--r-- (0644)', context: 'u:object_r:vendor_configs_file:s0', rebrandingTarget: 'ODM Manifest File', details: 'Lists hardware abstraction interfaces (HAL) registered for local devices.' },
      { name: 'firmware/soc_gpu.bin', files: 2, permission: 'r--r--r-- (0444)', context: 'u:object_r:vendor_firmware_file:s0', rebrandingTarget: 'GPU Firmware Binary', details: 'Microcode loading directly onto Adreno GPU inside Snapdragon 8 Elite chipset.' }
    ]
  }
};

function PartitionHierarchyD3({ selectedPartition, viewMode }: { selectedPartition: any, viewMode: 'tree' | 'sunburst' }) {
  ...
  useEffect(() => {
    ...
    const subPartitions = partitionDetails[selectedPartition.name as keyof typeof partitionDetails]?.subPartitions || [];
    ...
    if (viewMode === 'tree') {
      ...
      const node = svg.selectAll('.node')
        ...
        .on('mouseover', (event, d: any) => {
          ...
          d3.select(event.currentTarget)
            ...
            .style('transform-origin', `${d.y + margin.left}px ${d.x + margin.top}px`);
      ...
      const colors = ['#3B82F6', '#10B981', '#F59E0B', '#EC4899', '#818CF8', '#A78BFA'];
    ...
    <div className="bg-[#0B0F19] border border-[#22314D] p-4 rounded-xl relative overflow-hidden">
      <div className="flex items-center justify-between mb-2 min-h-[24px]">
        <span className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider">
          {viewMode === 'tree' ? '📂 Directory Node-Link Structure' : '📊 Radial File Proportional Area'}
        </span>
        {hoveredNode && (
          <div className="text-[10px] bg-slate-900/95 px-2 py-0.5 rounded border border-slate-700/50 flex items-center gap-2 animate-pulse font-mono">
            ...
            {hoveredNode.files !== undefined && (
              <span className="text-emerald-400 font-bold">({hoveredNode.files} files)</span>
            )}
            {hoveredNode.permission && (
              <span className="text-purple-400 font-semibold">{hoveredNode.permission}</span>
            )}
          </div>
        )}
      </div>
...
function FirmwarePartitions() {
  ...
  const [hierarchyViewMode, setHierarchyViewMode] = useState<'tree' | 'sunburst'>('tree');

  // Interactive partition state simulation
  const [encryptionStates, setEncryptionStates] = useState<Record<string, 'encrypted' | 'decrypted'>>({});
  const [integrityCheckStates, setIntegrityCheckStates] = useState<Record<string, 'idle' | 'verifying' | 'verified'>>({});
  ...
  const generateAndDownloadJSONSummary = () => {
    ...
    const details = partitionDetails[selectedPartition.name as keyof typeof partitionDetails];
    const isEncrypted = encryptionStates[selectedPartition.name] !== 'decrypted';
    ...
    const summary = {
      ...
      encryption_state: isEncrypted ? 'FBE AES-256 Encrypted' : 'Plaintext Decrypted',
      sub_partitions: details?.subPartitions.map(sub => ({
        name: sub.name,
  ...
  useEffect(() => {
    ...
    const partitions = [
      { name: 'system.img', label: '/system', size: 2900000000, percentage: 48.4, color: '#3B82F6', sha256: 'd8f36c561b34c264a91aef037e93081e7f3c1b002cbf7170104a081cf13f8992' },
      { name: 'product.img', label: '/product', size: 1500000000, percentage: 25.1, color: '#818CF8', sha256: 'f12a441e976cb3deca5e03fe01b3a58e5cbfe0128c94faee600df81acbe753d0' },
      { name: 'vendor.img', label: '/vendor', size: 1100000000, percentage: 18.4, color: '#A78BFA', sha256: 'b32a4e5e4184c1737e91d5bc7ea46a9e102f92f2b3e47acdf981df983a54b38d' },
      { name: 'odm.img', label: '/odm', size: 482000000, percentage: 8.1, color: '#64748B', sha256: '3c1a2c3d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b' }
    ];
  ...
  return (
    <div className="relative border border-[#22314D] bg-[#070B14] p-4 rounded-xl mt-4">
      <h4 className="text-xs font-extrabold text-white uppercase tracking-wider mb-3">super.img Partition Byte-Size Distribution (D3.js Visualization)</h4>
        ...
        {hoveredPartition && (
          ...
          >
            <div className="font-bold text-white uppercase flex justify-between gap-4">
              ...
              <span style={{ color: hoveredPartition.color }}>{hoveredPartition.percentage}%</span>
            </div>
            <div className="text-slate-400">Byte Size: {hoveredPartition.size.toLocaleString()} bytes</div>
            <div className="text-[10px] text-indigo-400 font-semibold animate-pulse mt-0.5">⚡ Click block to open deconstruction details</div>
            <div className="text-purple-400 break-all leading-normal">
              SHA-256:<br />
              <span className="text-[9px] text-slate-300">{hoveredPartition.sha256}</span>
            </div>
          </div>
        )}
      </div>
      ...
      {selectedPartition && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={() => setSelectedPartition(null)}>
          <div 
          ...
          >
            ...
            <div className="flex items-start justify-between border-b border-[#22314D] pb-4">
              <div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full" style={{ backgroundColor: selectedPartition.color }} />
                  <h3 className="text-base font-extrabold text-white uppercase tracking-wider">{selectedPartition.name} Deconstruction details</h3>
                </div>
                <p className="text-xs text-slate-400 mt-1">
                  Dynamic partition mount point: <code className="text-blue-400 font-mono font-semibold">{selectedPartition.label}</code>
                </p>
              </div>
              <button 
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-[#0B0F19] border border-[#22314D] p-3 rounded-xl">
                <span className="text-[9px] text-slate-500 uppercase font-extrabold tracking-wider block">Total Bytes</span>
                <span className="text-xs font-bold text-white font-mono block mt-1">{selectedPartition.size.toLocaleString()} B</span>
              </div>
              <div className="bg-[#0B0F19] border border-[#22314D] p-3 rounded-xl">
                <span className="text-[9px] text-slate-500 uppercase font-extrabold tracking-wider block">Total File Count</span>
                <span className="text-xs font-bold text-white font-mono block mt-1">
                  {partitionDetails[selectedPartition.name as keyof typeof partitionDetails]?.fileCount.toLocaleString() || 'N/A'}
                </span>
              </div>
              <div className="bg-[#0B0F19] border border-[#22314D] p-3 rounded-xl">
                <span className="text-[9px] text-slate-500 uppercase font-extrabold tracking-wider block">Security Integrity</span>
                <span className="text-xs font-bold text-emerald-400 font-mono block mt-1">
                  {partitionDetails[selectedPartition.name as keyof typeof partitionDetails]?.permissionsStatus || 'N/A'}
                </span>
              </div>
              <div className="bg-[#0B0F19] border border-[#22314D] p-3 rounded-xl">
                <span className="text-[9px] text-slate-500 uppercase font-extrabold tracking-wider block">SHA-256 Hash</span>
                <span className="text-[9px] font-bold text-indigo-400 font-mono truncate block mt-1" title={selectedPartition.sha256}>
              ...
              return (
                <div className="bg-[#0B0F19] border border-[#22314D] p-4 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-gradient-to-r from-[#0B0F19] to-[#0d162d]/40">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${isEncrypted ? 'bg-emerald-950/40 text-emerald-400' : 'bg-amber-950/40 text-amber-500'}`}>
                      {isEncrypted ? <Lock className="h-4.5 w-4.5 animate-pulse" /> : <Unlock className="h-4.5 w-4.5" />}
                    </div>
                    <div>
                      <span className="text-[9px] text-slate-400 font-extrabold uppercase block tracking-wider">Partition Encryption State</span>
                      <span className={`text-xs font-bold font-mono block ${isEncrypted ? 'text-emerald-400' : 'text-amber-500'}`}>
                        {isEncrypted ? '🔒 FBE AES-256-XTS Active (Hardware Wrapped)' : '🔓 Plaintext Decrypted (Security Keys Zeroed / Raw Access)'}
                      </span>
                    </div>
                  </div>
                  <button
                    onClick={() => {
                      setEncryptionStates(prev => ({
                        ...prev,
                        [selectedPartition.name]: isEncrypted ? 'decrypted' : 'encrypted'
                      }));
                    }}
                  ...
                  >
                    {isEncrypted ? '🔓 Decrypt Partition' : '🔒 Enable FBE Encryption'}
                  </button>
                </div>
              );
            })()}
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <h4 className="text-xs font-extrabold text-white uppercase tracking-wider">📦 D3.js Directory Hierarchy Visualizer</h4>
              ...
              <PartitionHierarchyD3 selectedPartition={selectedPartition} viewMode={hierarchyViewMode} />
            </div>

            {/* Sub-partition list */}
            <div className="space-y-3">
              <h4 className="text-xs font-extrabold text-white uppercase tracking-wider">Sub-Partition Components & File Permissions</h4>
              <div className="space-y-3 max-h-[30vh] overflow-y-auto pr-1">
                {partitionDetails[selectedPartition.name as keyof typeof partitionDetails]?.subPartitions.map((sub, idx) => {
                  const isEncrypted = encryptionStates[selectedPartition.name] !== 'decrypted';
                  return (
                    <div key={idx} className="bg-[#0B0F19] border border-[#22314D] p-4 rounded-xl space-y-2 hover:border-blue-500/30 transition-all">
                      <div className="flex flex-wrap justify-between items-center gap-2 border-b border-blue-950/40 pb-1.5">
                        <span className="text-xs font-mono font-bold text-blue-400">{sub.name}</span>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-[10px] leading-relaxed">
                        <div>
                          <span className="text-slate-500 font-bold block uppercase text-[8px]">SELinux Security Context</span>
                        ...
                        <div>
                          <span className="text-slate-500 font-bold block uppercase text-[8px]">Encryption Context</span>
                          <code className={`px-1 py-0.5 rounded border text-[9px] font-mono break-all inline-block mt-0.5 ${
                            isEncrypted 
                              ? 'text-indigo-400 bg-indigo-950/10 border-indigo-500/10' 
                              : 'text-yellow-400 bg-yellow-950/10 border-yellow-500/10'
                          }`}>
                            {isEncrypted ? 'aes-256-xts:fbe_policy' : 'decrypted_raw_inodes'}
                          </code>
                        </div>
                        {sub.rebrandingTarget ? (
                          <div>
                            <span className="text-slate-500 font-bold block uppercase text-[8px]">Rebranding Component</span>
                            <span className="text-emerald-400 font-bold block mt-0.5">
                              ⚙️ {sub.rebrandingTarget}
                            </span>
                          </div>
                        ) : (
                          <div>
                            <span className="text-slate-500 font-bold block uppercase text-[8px]">Hash Integrity Status</span>
                            {integrityCheckStates[sub.name] === 'verified' ? (
...
export default function RootMasterLab() {
  ...
  const defaultTasks: Task[] = [
    ...
    { id: 'RM-DIS-01', module: 'Dissection & Storage', title: 'Extract S25 Ultra Stock2026_05_04.rar firmware archive', desc: 'Verify RAR integrity and extract baseline AP, BL, CP, CSC tar packages.', priority: 'High', status: 'Pending' },
    { id: 'RM-DIS-02', module: 'Dissection & Storage', title: 'Extract super.img dynamic partition from AP tar.md5', desc: 'Utilize simg2img tool to convert sparse system imagery into raw mountable ext4 images.', priority: 'High', status: 'Pending' },
    { id: 'RM-DIS-03', module: 'Dissection & Storage', title: 'Unzip and analyze IML TOOL PRO FREE repair suite', desc: 'Identify system drivers, DLL payloads, and flash command endpoints.', priority: 'High', status: 'Pending' },
    { id: 'RM-DIS-04', module: 'Dissection & Storage', title: 'Parse firmware partition schema via IML dynamic reader', desc: 'Map logical blocks for system, vendor, odm, product, and boot partitions.', priority: 'High', status: 'Pending' },
    { id: 'RM-DIS-05', module: 'Dissection & Storage', title: 'Clear Android device internal storage using ADB shell', desc: 'Run logcat clear, purge /data/cache, and clean old log files to make room for 17GB stock file operations.', priority: 'Medium', status: 'Pending' },
    { id: 'RM-DIS-06', module: 'Dissection & Storage', title: 'Verify boot.img kernel signature & headers', desc: 'Analyze S938U boot image header version 4, extracting dtb (device tree blob) and ramdisk.', priority: 'High', status: 'Pending' },
    { id: 'RM-DIS-07', module: 'Dissection & Storage', title: 'Decompile bootloader BL_S938USQS3CXH2 trustlets', desc: 'Analyze Secure World TA (Trusted Applications) binaries managing Knox Vault keys.', priority: 'Low', status: 'Pending' },
    { id: 'RM-DIS-08', module: 'Dissection & Storage', title: 'Mount product.img and customize app overlays', desc: 'Identify bloating carrier apk lists and verify where overlay packages reside.', priority: 'Medium', status: 'Pending' },
    
    // MODULE 1: EXECUTIVE DASHBOARD
    { id: 'RM-DASH-01', module: 'Executive Dashboard', title: 'Build Project Lifecycle visual progress widgets', desc: 'Design clean Material 3 linear telemetry meters tracking overall MVP milestone states.', priority: 'High', status: 'Pending' },
    { id: 'RM-DASH-02', module: 'Executive Dashboard', title: 'Implement dynamic AI Diagnostic status indicators', desc: 'Add glowing widgets flashing real-time state changes for local neural firmware sweeps.', priority: 'Medium', status: 'Pending' },
    { id: 'RM-DASH-03', module: 'Executive Dashboard', title: 'Create interactive Gantt timeline grid component', desc: 'Visualize 3-year development plan containing critical path milestones.', priority: 'Low', status: 'Pending' },
    { id: 'RM-DASH-04', module: 'Executive Dashboard', title: 'Integrate active device trust score matrix stats', desc: 'Show distribution of client integrity values across current active fleet.', priority: 'Medium', status: 'Pending' },
    { id: 'RM-DASH-05', module: 'Executive Dashboard', title: 'Add real-time workspace threat feeds panel', desc: 'Display audit trail alarms on the dashboard with responsive flash animation.', priority: 'High', status: 'Pending' },

    // MODULE 2: DEVICE INVENTORY
    { id: 'RM-DEV-01', module: 'Device Inventory', title: 'Design Samsung Galaxy S25 Ultra (SM-S938U) profile specs', desc: 'Map critical attributes: Snapdragon 8 Elite SoC, Knox version 3.10, eSIM profile metadata.', priority: 'High', status: 'Pending' },
    { id: 'RM-DEV-02', module: 'Device Inventory', title: 'Configure Knox e-fuse hardware warranty tracker', desc: 'Read Knox status register. If 0x1 (warranty void), instantly update compliance to QUARANTINED.', priority: 'High', status: 'Pending' },
    { id: 'RM-DEV-03', module: 'Device Inventory', title: 'Verify Verizon custom radio frequency standard parameters', desc: 'Integrate RF diagnostics for LTE/5G bands (B13, B66, n2, n5, n77) matching CTIA limits.', priority: 'Medium', status: 'Pending' },
    { id: 'RM-DEV-04', module: 'Device Inventory', title: 'Implement system-wide Quarantine isolation workflows', desc: 'Allow admins to click isolation action, instantly severing tokens for substandard client states.', priority: 'High', status: 'Pending' },
    { id: 'RM-DEV-05', module: 'Device Inventory', title: 'Build dynamic device health telemetry list', desc: 'Expose CPU temperature, battery health cycle count, and RAM headroom statistics.', priority: 'Low', status: 'Pending' },

    // MODULE 3: FIRMWARE RESEARCH CENTER
    { id: 'RM-FIRM-01', module: 'Firmware Research', title: 'Map system.img file-system and verify SHA-256 hashes', desc: 'Extract block-level checksums from stock partition images, comparing against VRU3CXH2 baseline.', priority: 'High', status: 'Pending' },
    { id: 'RM-FIRM-02', module: 'Firmware Research', title: 'Create side-by-side firmware comparison workspace', desc: 'Build comparative UI comparing system headers, kernel build numbers, and security patches.', priority: 'Medium', status: 'Pending' },
    { id: 'RM-FIRM-03', module: 'Firmware Research', title: 'Develop delta-OTA update analyzer', desc: 'Parse binary diffs inside Verizon OTA payloads to track changes in driver layouts.', priority: 'Low', status: 'Pending' },
    { id: 'RM-FIRM-04', module: 'Firmware Research', title: 'Map SELinux rule compiler and check context bounds', desc: 'Scan sepolicy binaries in vendor partition to search for permissive rules or context loopholes.', priority: 'High', status: 'Pending' },
    { id: 'RM-FIRM-05', module: 'Firmware Research', title: 'Document RKP (Real-time Kernel Protection) hooks', desc: 'Trace RKP hypervisor intercepts designed to block credential modifications in memory.', priority: 'Medium', status: 'Pending' },

    // MODULE 4: AI ASSISTANT WORKSPACE
    { id: 'RM-AI-01', module: 'AI Operations & Logs', title: 'Build local firmware analysis prompt compiler', desc: 'Create specialized prompt constructors instructing Gemini to dissect system crash dumps.', priority: 'High', status: 'Pending' },
    { id: 'RM-AI-02', module: 'AI Operations & Logs', title: 'Design live Logcat parser with classification neural maps', desc: 'Classify incoming log streams into INFO, DEBUG, WARNING, and CRITICAL_KNOX buckets.', priority: 'High', status: 'Pending' },
    { id: 'RM-AI-03', module: 'AI Operations & Logs', title: 'Develop automatic script generator for Magisk injection', desc: 'Generate custom post-fs-data shell scripts targeting modular system mounting.', priority: 'Medium', status: 'Pending' },
    { id: 'RM-AI-04', module: 'AI Operations & Logs', title: 'Implement offline intelligence troubleshooting wiki', desc: 'Cache detailed knowledge base containing common recovery loop resolutions.', priority: 'Low', status: 'Pending' },

    // MODULE 5: ROOTMASTEROS DESIGN LAB
    { id: 'RM-DSN-01', module: 'RootMasterOS Design Lab', title: 'Apply One UI 8.1 geometric spacing system', desc: 'Style interface elements utilizing the 40-60 reachability rule and rounded 28dp card models.', priority: 'High', status: 'Pending' },
    { id: 'RM-DSN-02', module: 'RootMasterOS Design Lab', title: 'Build interactive Quick Settings toggles constructor', desc: 'Allow dragging, dropping, and configuring system buttons for Wi-Fi, Root Mode, and Safe Vault.', priority: 'Medium', status: 'Pending' },
    { id: 'RM-DSN-03', module: 'RootMasterOS Design Lab', title: 'Configure AMOLED-friendly pure black theme stylesheet', desc: 'Establish contrast bounds using background hex #000000 paired with vibrant blue #1A73FF.', priority: 'High', status: 'Pending' },
    { id: 'RM-DSN-04', module: 'RootMasterOS Design Lab', title: 'Simulate custom Boot Animation designer', desc: 'Upload PNG arrays to render a frame-by-frame loop representing Knox matrix boot streams.', priority: 'Low', status: 'Pending' },

    // MODULE 6: TESTING & COMPLIANCE
    { id: 'RM-TEST-01', module: 'Testing & Compliance', title: 'Draft CTIA 3.8.2 RF Signal compliance reports', desc: 'Generate official compliance records certifying total radiated power limits are achieved.', priority: 'High', status: 'Pending' },
    { id: 'RM-TEST-02', module: 'Testing & Compliance', title: 'Develop automated system-test plan execution scheduler', desc: 'Trigger dynamic verification lists evaluating API latency and biometric validation bounds.', priority: 'Medium', status: 'Pending' },
    { id: 'RM-TEST-03', module: 'Testing & Compliance', title: 'Build immutable blockchain audit trail logging queue', desc: 'Write cryptographically signed actions to local auditing tables for unalterable records.', priority: 'High', status: 'Pending' },
    { id: 'RM-TEST-04', module: 'Testing & Compliance', title: 'Integrate dynamic PDF report print export with Security Seal', desc: 'Ensure PDF reports dynamically overlay custom SVG Knox certified verification insignias.', priority: 'High', status: 'Pending' }
  ];
  ...
  useEffect(() => {
    ...
    if (savedSort === 'Priority' || savedSort === 'Status' || savedSort === 'None') {
  ...
  const [activeTab, setActiveTab] = useState<'backlog' | 'os-builder' | 'dissection' | 'storage' | 'manual'>('os-builder');
  ...
  const [selectedFirmwarePart, setSelectedFirmwarePart] = useState<'AP' | 'BL' | 'CP' | 'CSC' | null>('AP');
  ...
  const [successfulBuilds, setSuccessfulBuilds] = useState<any[]>(initialBuildLogsHistory);
  ...
  const [scannedFiles, setScannedFiles] = useState<Array<{
    ...
    { partition: '/system', filepath: 'system/lib64/libart.so', status: 'pending' },
    { partition: '/system', filepath: 'system/etc/hosts', status: 'pending' },
    { partition: '/system', filepath: 'system/framework/services.jar', status: 'pending' },
    { partition: '/system', filepath: 'system/etc/init.rc', status: 'pending' },
    { partition: '/system', filepath: 'system/bin/app_process64', status: 'pending' },
    { partition: '/system', filepath: 'system/framework/framework-res.apk', status: 'pending' },
    { partition: '/system', filepath: 'system/priv-app/SystemUI/SystemUI.apk', status: 'pending' },
    { partition: '/system', filepath: 'system/lib64/libc.so', status: 'pending' },
    { partition: '/system', filepath: 'system/etc/permissions/platform.xml', status: 'pending' },
    { partition: '/vendor', filepath: 'vendor/bin/hw/camera.service', status: 'pending' },
    { partition: '/vendor', filepath: 'vendor/lib/libunwind.so', status: 'pending' },
    { partition: '/vendor', filepath: 'vendor/etc/selinux/nonplat_sepolicy.cil', status: 'pending' },
    { partition: '/vendor', filepath: 'vendor/bin/hw/android.hardware.graphics.allocator@4.0-service', status: 'pending' },
    { partition: '/vendor', filepath: 'vendor/etc/init/hw/init.target.rc', status: 'pending' },
    { partition: '/vendor', filepath: 'vendor/firmware/keymaster.b01', status: 'pending' },
    { partition: '/vendor', filepath: 'vendor/manifest.xml', status: 'pending' },
    { partition: '/product', filepath: 'product/app/PrebuiltGmail.apk', status: 'pending' },
    { partition: '/product', filepath: 'product/overlay/NoOverlays.apk', status: 'pending' },
    { partition: '/product', filepath: 'product/etc/sysconfig/com.android.hotspot2.xml', status: 'pending' },
    { partition: '/product', filepath: 'product/media/bootanimation.zip', status: 'pending' },
    { partition: '/product', filepath: 'product/framework/product-framework.jar', status: 'pending' },
    { partition: '/product', filepath: 'product/priv-app/CarrierConfig/CarrierConfig.apk', status: 'pending' },
    { partition: '/odm', filepath: 'odm/etc/wifi/nvram.txt', status: 'pending' },
    { partition: '/odm', filepath: 'odm/lib64/libodm_hardware.so', status: 'pending' },
    { partition: '/odm', filepath: 'odm/etc/permissions/sku_vendor.xml', status: 'pending' },
    { partition: '/odm', filepath: 'odm/firmware/adsp.mdt', status: 'pending' },
    { partition: '/odm', filepath: 'odm/bin/hw/vendor.samsung.hardware.security.vault@1.0-service', status: 'pending' },
  ]);
    ...
    const interval = setInterval(() => {
      if (currentIdx < fileCount) {
        setScannedFiles(prev => {
          ...
          const finalStatus = (file.filepath === 'system/framework/services.jar' || file.filepath === 'vendor/etc/selinux/nonplat_sepolicy.cil') 
            ? 'corrupted' 
            : 'verified';
        ...
        triggerNotification("Partition Integrity Check Complete! Security alerts raised for corrupted system and vendor payloads.");
      }
    }, 180);
  };
  ...
  const resetToDefault = () => {
    if (confirm("Are you sure you want to restore the master task backlog? This will overwrite your current progress.")) {
  ...
  const runOSBuildAssembly = () => {
    ...
    if (!selectedZipFiles.rootmaster || !selectedZipFiles.allinone || !selectedZipFiles.download) {
      triggerNotification("Error: You must check and verify all three ZIP archives first.");
      return;
    }
    ...
    const runStage4 = () => {
      ...
      const s4Interval = setInterval(() => {
        if (index < s4Logs.length) {
          ...
          triggerNotification("RootMasterOS bootable ISO successfully compiled!");
          ...
          setSuccessfulBuilds(prev => {
            ...
            const newBuildLogs = [
              ...
              "[STAGE 4] COMPRESSING FILESYSTEM & REBUILDING GRUB BOOT SYSTEM",
              "  - Compression ratio: 4.8:1 - Reduced 2.8GB system workspace into 582MB SquashFS image.",
              "[COMPLETED] BOOTABLE ROOTMASTEROS ISO FULLY COMPILED (" + nextId + ")",
              "⚡ ISO NAME: RootMasterOS_" + nextId.replace(' ', '_').toLowerCase() + ".iso",
              "⚡ SIZE: 642 MB",
              "⚡ STATUS: Ready to flash"
            ];
            return [
  ...
  const exportLogHistoryZip = () => {
    ...
    try {
      ...
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      triggerNotification(`All ${successfulBuilds.length} build logs exported as ZIP successfully!`);
    } catch (error) {
      console.error(error);
  ...
  const exportBuildLogsZip = () => {
    const logsToExport = osBuildLogs.length > 0 ? osBuildLogs : [
      "Initializing RootMasterOS Bootable OS Builder Environment...",
      "[SYSTEM] Host Architecture detected: x86_64, Linux Kernel Baseline v6.8.0",
    ...
    const getZipBlob = (filename: string, content: string) => {
      ...
      return new Blob([lhBytes, cdBytes, eocdBytes], { type: 'application/zip' });
    };
    ...
    const runAPDissection = () => {
      ...
      const apInterval = setInterval(() => {
        if (index < apLogs.length) {
          ...
          triggerNotification("AP Partition dissection complete! Dynamic sub-partitions exposed.");
        }
      }, 350);
    };
    ...
    const runBLDissection = () => {
      ...
      const blInterval = setInterval(() => {
        if (index < blLogs.length) {
          ...
          setIsDissecting(false);
          triggerNotification("Bootloader signatures mapped and Knox registers exposed.");
        }
      }, 400);
    };
  ...
  const runStorageOptimization = () => {
    ...
    const interval = setInterval(() => {
      if (index < actions.length) {
        ...
        triggerNotification("S25 Ultra storage optimized successfully! 21.5 GB freed.");
      }
    }, 400);
  };
        ...
        <div className="relative z-10 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
          <div className="space-y-2 max-w-4xl">
            ...
            <p className="text-sm font-medium text-slate-400 leading-relaxed max-w-3xl">
              Construct a real, bootable Ubuntu-based live Linux ISO embedding RootMasterOS, All-in-One dashboards, and Magisk Android modules. Complete with interactive simulators for Samsung S25 Ultra Odin firmware partition reverse-engineering, IML tools, and storage optimizations.
            </p>
          </div>
              ...
              <div className="glass-card p-6 rounded-2xl border border-[#22314D] space-y-6">
                <div className="flex items-center justify-between border-b border-[#22314D] pb-4">
                  <div className="flex items-center gap-2.5">
                    ...
                    <h3 className="text-sm font-extrabold text-white uppercase tracking-wider">Live ISO Builder Environment</h3>
                  </div>
                    ...
                    <div className="p-4 rounded-xl border border-[#22314D] bg-[#070B14] hover:border-blue-500/30 transition-all flex flex-col justify-between space-y-3">
                      <div className="flex items-start justify-between">
                        ...
                        <input 
                          ...
                          onChange={(e) => setSelectedZipFiles({ ...selectedZipFiles, rootmaster: e.target.checked })}
                          className="rounded text-blue-500 bg-slate-900 border-slate-700 h-4.5 w-4.5 focus:ring-0"
                        />
                      </div>
                      <div>
                        <span className="block text-xs font-bold text-white">RootMasterOS.zip</span>
                        <span className="block text-[10px] text-slate-500 mt-0.5">Base Distro & Express Files</span>
                      </div>
                    ...
                    <div className="p-4 rounded-xl border border-[#22314D] bg-[#070B14] hover:border-purple-500/30 transition-all flex flex-col justify-between space-y-3">
                      <div className="flex items-start justify-between">
                        ...
                        <input 
                          ...
                          onChange={(e) => setSelectedZipFiles({ ...selectedZipFiles, allinone: e.target.checked })}
                          className="rounded text-blue-500 bg-slate-900 border-slate-700 h-4.5 w-4.5 focus:ring-0"
                        />
                      </div>
                      <div>
                        <span className="block text-xs font-bold text-white">All-in-One.zip</span>
                        <span className="block text-[10px] text-slate-500 mt-0.5">Unified Web Control Interface</span>
                      </div>
                      <span className="text-[9px] text-emerald-400 font-bold uppercase tracking-wider flex items-center gap-1">
                        <CheckCircle2 className="h-3.5 w-3.5" /> VERIFIED OK
                      </span>
                    ...
                    <div className="p-4 rounded-xl border border-[#22314D] bg-[#070B14] hover:border-purple-500/30 transition-all flex flex-col justify-between space-y-3">
                      <div className="flex items-start justify-between">
                        ...
                        <input 
                          ...
                          onChange={(e) => setSelectedZipFiles({ ...selectedZipFiles, download: e.target.checked })}
                          className="rounded text-blue-500 bg-slate-900 border-slate-700 h-4.5 w-4.5 focus:ring-0"
                        />
                      </div>
                      <div>
                        <span className="block text-xs font-bold text-white">Download.zip (bindhosts)</span>
                        <span className="block text-[10px] text-slate-500 mt-0.5">Magisk Knox Android Module</span>
                      </div>
                      <span className="text-[9px] text-emerald-400 font-bold uppercase tracking-wider flex items-center gap-1">
                        <CheckCircle2 className="h-3.5 w-3.5" /> VERIFIED OK
                      </span>
                ...
                <div className="p-5 rounded-2xl border border-[#22314D] bg-[#070B14] space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#22314D]/40 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h4 className="text-xs font-extrabold text-white uppercase tracking-wider">Isolated Build Sandbox Allocation</h4>
                        <p className="text-[10px] text-slate-400 mt-0.5">Adjust the dedicated workspace memory limits for compiling files safely.</p>
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-2">
                      <span className={`text-[9px] font-bold px-2 py-0.5 rounded uppercase font-mono ${
                        isResizingSandbox ? 'bg-amber-950/40 text-yellow-400 animate-pulse border border-yellow-800/20' : 'bg-blue-950/40 text-blue-400 border border-blue-900/20'
                      }`}>
                        {isResizingSandbox ? '⚡ REALLOCATING BOUNDS...' : 'STATUS: OPTIMIZED'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="text-xs font-bold text-slate-300">
                        Virtual Storage Limit: <span className="text-indigo-400 font-extrabold font-mono text-sm">{sandboxSize} GB</span>
                      </div>
                      <div className="text-[10px] text-slate-500 font-extrabold uppercase">Min: 4GB / Max: 64GB</div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      <input 
                        ...
                        max="64" 
                        value={sandboxSize} 
                        onChange={(e) => handleSandboxResize(parseInt(e.target.value))}
                    ...
                    <div className="grid grid-cols-3 gap-3 pt-1">
                      <div className="p-3 bg-[#0B0F19] rounded-xl border border-[#22314D]/40 text-center">
                        <span className="text-[8px] text-slate-500 font-bold block uppercase">SquashFS RAM</span>
                        <span className="text-xs font-mono font-bold text-white">{(sandboxSize * 0.45).toFixed(1)} GB</span>
                      </div>
                      <div className="p-3 bg-[#0B0F19] rounded-xl border border-[#22314D]/40 text-center">
                        <span className="text-[8px] text-slate-500 font-bold block uppercase">Decompress Temp</span>
                        <span className="text-xs font-mono font-bold text-white">{(sandboxSize * 0.35).toFixed(1)} GB</span>
                      </div>
                      <div className="p-3 bg-[#0B0F19] rounded-xl border border-[#22314D]/40 text-center">
                        <span className="text-[8px] text-slate-500 font-bold block uppercase">Unallocated Gap</span>
                        <span className="text-xs font-mono font-bold text-emerald-400">{(sandboxSize * 0.20).toFixed(1)} GB</span>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      <h4 className="text-xs font-extrabold text-white uppercase tracking-wider flex items-center gap-1.5">
                        <Gauge className="h-4 w-4 text-blue-400" /> Compile Engine Status
                      </h4>
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        Construct custom Ubuntu-Live squashfs filesystem injecting your merged files & GRUB menu entry.
                      </p>
                    </div>
                ...
                <div className="p-5 font-mono text-[11px] leading-relaxed text-blue-400/90 h-96 overflow-y-auto scrollbar-thin flex flex-col space-y-1">
                  {osBuildLogs.length === 0 ? (
                    <div className="text-slate-500 text-center py-20 flex flex-col items-center justify-center space-y-3">
                      ...
                      <span>Console idle. Click "Assemble Bootable OS" to begin compilation stream.</span>
                    </div>
                  ) : (
                      ...
                      <div 
                        ...
                        className={
                          log.includes("[STAGE") || log.includes("[COMPLETED") 
                            ...
                            : log.includes("Success") || log.includes("VERIFIED OK")
            ...
            <div className="space-y-6">
              ...
              {isoResultReady ? (
                <div className="p-6 rounded-2xl border-2 border-emerald-500 bg-emerald-950/10 shadow-[0_0_25px_rgba(16,185,129,0.1)] space-y-5 animate-pulse-slow">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-xs font-extrabold text-white uppercase tracking-wider">COMPILATION SUCCESSFUL</h4>
                      <p className="text-[10px] text-slate-400">Firmware OS package compiled.</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 p-4 rounded-xl bg-slate-950 border border-slate-900 font-mono text-[10px] text-slate-300">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-white">RootMasterOS.iso</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-white">cf83a9032d91...</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">KERNEL:</span>
                      <span className="font-bold text-white">Linux 6.8-generic</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">DESKTOP:</span>
                      <span className="font-bold text-white">XFCE UI Light</span>
                    </div>
                  </div>

                  <div className="space-y-2.5">
                    <button 
                      onClick={() => triggerNotification("RootMasterOS.iso download started!")}
                    ...
                    <button 
                      onClick={() => handleCopyToClipboard("sudo dd if=RootMasterOS.iso of=/dev/sdX bs=4M status=progress", "DD Flash Command")}
                ...
                <div className="p-6 rounded-2xl border border-[#22314D] bg-[#070B14] space-y-4 text-center">
                  ...
                  <h4 className="text-xs font-extrabold text-white uppercase tracking-wider">No Iso File Generated Yet</h4>
                  <p className="text-[11px] text-slate-400">
                    Once the Live ISO build pipeline completes, the custom bootable system image will populate here with credentials and checksum hashes.
                  </p>
                </div>
              )}

              {/* Build History Performance Chart using Recharts */}
              <BuildHistoryChart successfulBuilds={successfulBuilds} onExportHistory={exportLogHistoryZip} />
              ...
              <div className="glass-card p-6 rounded-2xl border border-[#22314D] space-y-4">
                <div className="flex items-center gap-2.5 border-b border-[#22314D] pb-3">
                  ...
                  <h3 className="text-xs font-extrabold text-white uppercase tracking-wider">Advanced OS Integrations</h3>
                </div>

                <div className="space-y-3.5 text-xs">
                  <p className="text-[11px] text-slate-400 leading-normal">
                    You can scale this dynamic framework into several robust custom distribution profiles by replacing the base debootstrap target or overlays:
                  </p>

                  <div className="space-y-2.5">
                    <div className="p-3 rounded-xl border border-blue-950/30 bg-blue-950/10 hover:border-blue-500/30 transition-all cursor-pointer" onClick={() => triggerNotification("Upgrade Profile: Kali Baseline configured.")}>
                      <strong className="text-white block text-[11px]">🧬 1. Kali Security Linux Baseline</strong>
                      <span className="block text-[10px] text-slate-400 mt-0.5 leading-normal">
                        Inject cybersecurity audit tools (nmap, aircrack-ng, firmware-mod-kit) straight into the live SquashFS.
                      </span>
                    </div>

                    <div className="p-3 rounded-xl border border-purple-950/30 bg-purple-950/10 hover:border-purple-500/30 transition-all cursor-pointer" onClick={() => triggerNotification("Upgrade Profile: Persistent Live storage enabled.")}>
                      <strong className="text-white block text-[11px]">💾 2. Live Persistence storage</strong>
                      <span className="block text-[10px] text-slate-400 mt-0.5 leading-normal">
                        Utilize dual-partition flashing setups. Partition A loads live SquashFS; Partition B auto-mounts as crypt-fs home space.
                      </span>
                    </div>

                    <div className="p-3 rounded-xl border border-emerald-950/30 bg-emerald-950/10 hover:border-emerald-500/30 transition-all cursor-pointer" onClick={() => triggerNotification("Upgrade Profile: Android SDK & ADB Bridge set up.")}>
                      <strong className="text-white block text-[11px]">📱 3. Unified Android SDK Bridge</strong>
                      <span className="block text-[10px] text-slate-400 mt-0.5 leading-normal">
                        Include ADB client, fastboot triggers, and a local headless Android Emulator to execute Magisk payload sweeps natively.
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="glass-card p-6 rounded-2xl border border-[#22314D] space-y-6">
                <div>
                  <h3 className="text-sm font-extrabold text-white uppercase tracking-wider flex items-center gap-1.5">
                    <Layers className="h-4.5 w-4.5 text-blue-400" /> S25 Ultra Firmware Package Deconstructor
                  </h3>
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    Target baseline: <code className="text-blue-400 font-mono">S938USQS3CXH2</code> (Android 16, binary 3, CSC: VZW Verizon).
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {['AP', 'BL', 'CP', 'CSC'].map(part => (
                    ...
                    >
                      <span className="block text-lg font-black text-white">{part}</span>
                      <span className="block text-[9px] text-slate-400 uppercase font-bold tracking-wider leading-relaxed">
                        {part === 'AP' ? 'System & APPs' : part === 'BL' ? 'Secure Bootloader' : part === 'CP' ? 'Modem Baseband' : 'Customizations'}
                      </span>
                      <span className="block text-[10px] text-slate-500 font-mono font-semibold">
                        {part === 'AP' ? '12.8 GB' : part === 'BL' ? '32.1 MB' : part === 'CP' ? '91.4 MB' : '482 MB'}
                      </span>
                    </button>
                  ))}
                </div>
                  ...
                  <div className="space-y-4">
                    <div className="p-4 rounded-xl border border-blue-900/40 bg-blue-950/20 text-xs text-slate-300 leading-relaxed space-y-1.5">
                      <strong className="text-white block font-extrabold">
                        Selected Target details: {selectedFirmwarePart === 'AP' ? 'AP_S938USQS3CXH2_AP.tar.md5' : selectedFirmwarePart === 'BL' ? 'BL_S938USQS3CXH2_BL.tar.md5' : selectedFirmwarePart === 'CP' ? 'CP_S938USQS3CXH2_CP.tar.md5' : 'CSC_OMC_VZW_S938USQS3CXH2.tar.md5'}
                      </strong>
                    ...
                    <div className="space-y-2.5 p-4 rounded-xl border border-[#22314D] bg-[#070B14]">
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">
                          Payload Archive Contents ({selectedFirmwarePart === 'AP' ? '12' : selectedFirmwarePart === 'BL' ? '9' : selectedFirmwarePart === 'CP' ? '2' : '4'} files)
                        </span>
                      ...
                      <div className="max-h-[120px] overflow-y-auto pr-1.5 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-[#070B14] space-y-1 text-[11px] font-mono">
                        {selectedFirmwarePart === 'AP' && [
                          ...
                          <div key={f.name} className="flex justify-between items-center p-2 rounded bg-[#030712]/50 border border-[#22314D]/30 hover:border-blue-500/30 transition-all">
                        ...
                        {selectedFirmwarePart === 'BL' && [
                          ...
                          <div key={f.name} className="flex justify-between items-center p-2 rounded bg-[#030712]/50 border border-[#22314D]/30 hover:border-purple-500/30 transition-all">
                            <span className="text-white font-bold">{f.name}</span>
                            <div className="flex gap-3 text-slate-400">
                              <span>{f.size}</span>
                              <span className="text-emerald-400 text-[10px] font-bold">OK</span>
                        ...
                        {selectedFirmwarePart === 'CSC' && [
                          ...
                          <div key={f.name} className="flex justify-between items-center p-2 rounded bg-[#030712]/50 border border-[#22314D]/30 hover:border-emerald-500/30 transition-all">
                            <span className="text-white font-bold">{f.name}</span>
                            <div className="flex gap-3 text-slate-400">
                              <span>{f.size}</span>
                              <span className="text-emerald-400 text-[10px] font-bold">OK</span>
                ...
                <div className="flex justify-between items-center pt-4 border-t border-[#22314D]">
                  <span className="text-[10px] text-slate-400 font-bold uppercase flex items-center gap-1">
                    <Info className="h-4 w-4" /> Ready to decompile and verify headers
                  </span>
                  ...
                  >
                    {isDissecting ? 'Dissecting Partition...' : 'Run Decompile Sweep'}
                  </button>
                </div>
              </div>
              ...
              <div className="glass-card rounded-2xl border border-[#22314D] bg-[#020617] overflow-hidden shadow-2xl">
                <div className="flex items-center justify-between bg-[#0B0F19] px-5 py-3 border-b border-[#22314D]">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-mono text-slate-400">{dissectProgress}%</span>
                  </div>
                </div>
                ...
                <div className="p-5 font-mono text-[11px] leading-relaxed text-indigo-400 h-80 overflow-y-auto scrollbar-thin flex flex-col space-y-1">
                  {dissectLogs.length === 0 ? (
                    <div className="text-slate-500 text-center py-20">
                      Terminal empty. Select a firmware block and click "Run Decompile Sweep" to view dynamic decompression, sparse conversions, and sha256 checksum maps.
                    </div>
                  ) : (
                      ...
                      <div 
                        ...
                        className={
                          log.includes("[COMPLETED") 
                            ...
                            : log.includes("Error") 
                            ? "text-red-500 font-extrabold" 
                            : log.includes("[MATCHED]") || log.includes("SUCCESSFUL")
                            ? "text-emerald-400 font-bold"
                            : "text-slate-300"
                        }
                      >
              ...
              <div className="glass-card p-6 rounded-2xl border border-[#22314D] bg-[#070B14] space-y-4">
                <div className="flex items-center justify-between border-b border-[#22314D] pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs font-extrabold text-white uppercase tracking-wider">Dynamic Partition Integrity Scan</h3>
                  </div>
                  <button
                  ...
                  >
                    ...
                    <span>{integrityScanActive ? 'Scanning...' : 'Start Integrity Scan'}</span>
                  </button>
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center text-[10px] text-slate-400 font-bold">
                    <span className="font-mono">{currentScanningFile || 'Ready to analyze system partitions'}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  {['/system', '/vendor', '/product', '/odm'].map(part => {
                    const partFiles = scannedFiles.filter(f => f.partition === part);
                    const verifiedCount = partFiles.filter(f => f.status === 'verified').length;
                    const corruptedCount = partFiles.filter(f => f.status === 'corrupted').length;
                    
                    return (
                      <div key={part} className="p-4 bg-[#0B0F19]/60 rounded-xl border border-[#22314D]/50 space-y-3">
                        <div className="flex justify-between items-center border-b border-[#22314D]/30 pb-1.5">
                          <span className="text-[11px] font-black text-white">{part} files</span>
                          <div className="flex gap-2 text-[9px] font-extrabold">
                            {verifiedCount > 0 && <span className="text-emerald-400 font-mono">{verifiedCount} OK</span>}
                            {corruptedCount > 0 && <span className="text-red-400 font-mono">{corruptedCount} FAIL</span>}
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5 max-h-[125px] overflow-y-auto pr-1.5 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-[#070B14]">
                          {partFiles.map(file => (
                            <div key={file.filepath} className="flex justify-between items-center text-[10px] font-mono py-1 border-b border-[#22314D]/20 last:border-0">
                              <span className="text-slate-400 truncate max-w-[150px]" title={file.filepath}>
            ...
            <div className="space-y-6">
              ...
              <div className="glass-card p-6 rounded-2xl border border-[#22314D] space-y-4">
                <div className="flex items-center justify-between border-b border-[#22314D] pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs font-extrabold text-white uppercase tracking-wider">super.img Extracted Map</h3>
                  </div>
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded ${
                    superImageUnpacked ? 'bg-emerald-950 text-emerald-400' : 'bg-slate-900 text-slate-500'
                  }`}>
                ...
                <div className="space-y-2 text-xs font-mono">
                  ...
                  <div className={`p-3 rounded-xl border transition-all ${
                    superImageUnpacked ? 'border-blue-900/40 bg-blue-950/5' : 'border-[#22314D] bg-[#070B14]'
                  }`}>
                    <div className="flex justify-between items-center text-white">
                      <span className="font-bold text-blue-400 text-[11px]">/system (system.img)</span>
                      <span className="text-[9px] text-slate-400">2.9 GB</span>
                    </div>
                    <span className="block text-[9px] text-slate-500 mt-1">
                      Contains core Android framework (init, bin, app system configs).
                    </span>
                  </div>

                  {/* Vendor.img Block */}
                  <div className={`p-3 rounded-xl border transition-all ${
                    superImageUnpacked ? 'border-purple-900/40 bg-purple-950/5' : 'border-[#22314D] bg-[#070B14]'
                  }`}>
                    <div className="flex justify-between items-center text-white">
                      <span className="font-bold text-purple-400 text-[11px]">/vendor (vendor.img)</span>
                      <span className="text-[9px] text-slate-400">1.1 GB</span>
                    </div>
                    <span className="block text-[9px] text-slate-500 mt-1">
                      Snapdragon 8 Elite hardware drivers and sepolicy security contexts.
                    </span>
                  </div>

                  {/* Product.img Block */}
                  <div className={`p-3 rounded-xl border transition-all ${
                    superImageUnpacked ? 'border-indigo-900/40 bg-indigo-950/5' : 'border-[#22314D] bg-[#070B14]'
                  }`}>
                    <div className="flex justify-between items-center text-white">
                      <span className="font-bold text-indigo-400 text-[11px]">/product (product.img)</span>
                  ...
                  {/* Odm.img Block */}
                  <div className={`p-3 rounded-xl border transition-all ${
                    superImageUnpacked ? 'border-slate-800 bg-slate-950/20' : 'border-[#22314D] bg-[#070B14]'
                  }`}>
                    <div className="flex justify-between items-center text-white">
                      <span className="font-bold text-slate-300 text-[11px]">/odm (odm.img)</span>
                      <span className="text-[9px] text-slate-400">482 MB</span>
                    </div>
                    <span className="block text-[9px] text-slate-500 mt-1">
                      Samsung hardware custom integrations and carrier descriptors.
                    </span>
                  </div>
              ...
              <div className="glass-card p-6 rounded-2xl border border-[#22314D] space-y-4">
                <div className="flex items-center gap-2 border-b border-[#22314D] pb-3">
                  ...
                  <h3 className="text-xs font-extrabold text-white uppercase tracking-wider">Samsung Knox Safety Attestation</h3>
                </div>
                ...
                <div className="space-y-3 font-mono text-[10px] text-slate-300">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-slate-400">LOCKED (CARRIER SPEC)</span>
                  </div>
                  ...
                  <p className="text-[10px] leading-relaxed text-slate-500 font-sans mt-2 pt-2 border-t border-[#22314D]">
                    Note: Writing compiled packages directly voids the Knox Bit instantly! Run boot verification loops first inside isolated custom OS VM containers.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="xl:col-span-2 space-y-6">
              ...
              <div className="glass-card p-6 rounded-2xl border border-[#22314D] space-y-6">
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-sm font-extrabold text-white uppercase tracking-wider flex items-center gap-1.5">
                      <HardDrive className="h-4.5 w-4.5 text-blue-400" /> S25 Ultra Space Optimizer
                    </h3>
                    <p className="text-[11px] text-slate-400 mt-0.5">
                      Clear cache blocks via ADB shell pm commands to make space for 17GB stock file operations.
                    </p>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="p-4 rounded-xl border border-[#22314D] bg-[#070B14]">
                      <span className="text-[9px] text-slate-500 font-extrabold uppercase block tracking-wider">TOTAL MEMORY</span>
                      <span className="text-xl font-black text-white">{storageStats.total} GB</span>
                    </div>

                    <div className="p-4 rounded-xl border border-[#22314D] bg-[#070B14]">
                      <span className="text-[9px] text-slate-500 font-extrabold uppercase block tracking-wider">USED SPACE</span>
                      <span className="text-xl font-black text-white">{storageStats.used} GB</span>
                    </div>

                    <div className="p-4 rounded-xl border border-[#22314D] bg-[#070B14]">
                      <span className="text-[9px] text-slate-500 font-extrabold uppercase block tracking-wider">FREE STORAGE</span>
                      <span className="text-xl font-black text-emerald-400">{storageStats.free} GB</span>
                    </div>

                    <div className="p-4 rounded-xl border border-[#22314D] bg-[#070B14]">
                      <span className="text-[9px] text-slate-500 font-extrabold uppercase block tracking-wider">STAGNANT CACHES</span>
                      <span className={`text-xl font-black ${storageStats.trashCaches > 0 ? 'text-yellow-500' : 'text-emerald-400'}`}>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-[10px] text-slate-500 font-bold">
                      ...
                      <span>{Math.round((storageStats.used / storageStats.total) * 100)}% ALLOCATED</span>
                    </div>
                    <div className="w-full bg-[#070B14] h-3.5 rounded-full overflow-hidden border border-[#22314D] flex">
                      <div className="bg-blue-600 h-full" style={{ width: `${Math.round((storageStats.used / storageStats.total) * 100)}%` }}></div>
                      <div className="bg-yellow-500 h-full" style={{ width: `${Math.round((storageStats.trashCaches / storageStats.total) * 100)}%` }}></div>
                    </div>
                    <div className="flex gap-4 text-[9px] text-slate-500 font-bold uppercase mt-1">
                      <div className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-blue-600"></span> Active Files</div>
                      <div className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-yellow-500"></span> Stagnant Caches</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center pt-4 border-t border-[#22314D]">
                  <span className="text-[10px] text-slate-400 font-bold uppercase flex items-center gap-1">
                    <Info className="h-4 w-4" /> Ready to invoke pm trim-caches blocks
                  </span>
                  ...
                  >
                    {optimizingStorage ? 'Optimizing Space...' : 'Run Cache Optimizer'}
                  </button>
                </div>
              </div>
                      ...
                      <div 
                        ...
                        className={
                          log.includes("[SUCCESS") 
                            ...
                            : log.includes("Error") 
                            ? "text-red-500 font-extrabold" 
                            : log.includes("adb shell") || log.includes("trim")
                            ? "text-yellow-400"
                            : "text-slate-300"
                        }
                      >
            ...
            <div className="space-y-6">
              ...
              <div className="glass-card p-6 rounded-2xl border border-[#22314D] space-y-4">
                <div className="flex items-center gap-2 border-b border-[#22314D] pb-3">
                  ...
                  <h3 className="text-xs font-extrabold text-white uppercase tracking-wider">Useful ADB Storage Codes</h3>
                </div>

                <div className="space-y-3.5 text-xs text-slate-300">
                  <p className="text-[11px] text-slate-400 leading-normal">
                    Manually execute space optimizations inside the S25 Ultra shell with these validated terminal syntaxes:
                  </p>

                  <div className="space-y-3 font-mono text-[10px]">
                    <div>
                      <span className="text-white font-bold">// 1. Free dynamic packages cache (e.g. 10G)</span>
                    ...
                    <div>
                      <span className="text-white font-bold">// 2. Trigger hardware storage FSTRIM routine</span>
                      <code className="block bg-[#070B14] border border-[#22314D] p-2 rounded mt-1 text-purple-400">
                    ...
                    <div>
                      <span className="text-white font-bold">// 3. Purge Android system logcat buffers</span>
                      <code className="block bg-[#070B14] border border-[#22314D] p-2 rounded mt-1 text-purple-400">
                        adb logcat -c
                      </code>
                    </div>
                    ...
                    <div>
                      <span className="text-white font-bold">// 4. Clean user application temporary spaces</span>
                      <code className="block bg-[#070B14] border border-[#22314D] p-2 rounded mt-1 text-purple-400">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="glass-card p-6 rounded-2xl flex flex-col justify-between border border-[#22314D] hover:border-[#2F58CD]/30 transition-all duration-300">
              <div className="space-y-1">
                ...
                <h2 className="text-2xl font-extrabold text-white tracking-tight">{completionPercentage}%</h2>
              </div>
              <div className="mt-4 space-y-2">
                <div className="w-full bg-[#111827] h-2 rounded-full overflow-hidden border border-[#22314D]">
                  <div className="bg-gradient-to-r from-[#2F58CD] to-[#10B981] h-full rounded-full transition-all duration-500" style={{ width: `${completionPercentage}%` }}></div>
                </div>
                <div className="flex justify-between text-[10px] text-slate-500 font-bold">
            ...
            <div className="glass-card p-6 rounded-2xl flex flex-col justify-between border border-[#22314D] hover:border-slate-700 transition-all duration-300">
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                  <ClipboardList className="h-3.5 w-3.5 text-slate-400" /> Pending Tasks
                </span>
                <h2 className="text-2xl font-extrabold text-white tracking-tight">{pendingCount}</h2>
              </div>
            ...
            <div className="glass-card p-6 rounded-2xl flex flex-col justify-between border border-[#22314D] hover:border-[#6C3483]/30 transition-all duration-300">
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                  <Sliders className="h-3.5 w-3.5 text-[#6C3483]" /> Under Active Trace
                </span>
                <h2 className="text-2xl font-extrabold text-white tracking-tight">{inProgressCount}</h2>
              </div>
            ...
            <div className="glass-card p-6 rounded-2xl flex flex-col justify-between border border-[#22314D] hover:border-[#10B981]/30 transition-all duration-300">
              <div className="space-y-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                  <CheckCircle2 className="h-3.5 w-3.5 text-[#10B981]" /> Attested compliance
                </span>
                <h2 className="text-2xl font-extrabold text-white tracking-tight">{completedCount}</h2>
              </div>
              ...
              <div className="space-y-3.5 max-h-[580px] overflow-y-auto pr-2 scrollbar-thin">
                {filteredAndSortedTasks.length === 0 ? (
                  <div className="glass-card p-12 text-center rounded-2xl border border-[#22314D]">
                    ...
                    <h3 className="text-sm font-bold text-white">No Matching Backlog Tasks Found</h3>
                    <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                      Refine your keyword search or change module/status filters to view corresponding program records.
                    </p>
                  </div>
                ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 space-y-1.5">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="text-[10px] font-mono text-slate-500 font-extrabold">{task.id}</span>
                          ...
                          <h4 className={`text-xs font-bold text-white transition-all ${isCompleted ? 'line-through text-slate-500' : ''}`}>
                            {task.title}
                          </h4>
                        ...
                        <div className="flex flex-col gap-1 self-start">
                          <button
                            onClick={() => handleCopyToClipboard(JSON.stringify(task, null, 2), `Task ${task.id}`)}
            ...
            <div className="space-y-6">
              ...
              <div className="glass-card p-6 rounded-2xl border border-[#22314D] space-y-4">
                <div className="flex items-center gap-2 border-b border-[#22314D] pb-3">
                  ...
                  <h3 className="text-xs font-extrabold text-white uppercase tracking-wider">Add Custom Blueprint Task</h3>
                </div>
                ...
                <form onSubmit={handleAddTask} className="space-y-3.5">
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Module / Category</label>
                    <select 
                      value={newModule}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Detailed description (Optional)</label>
                    <textarea 
                      value={newDesc}
          ...
          <div className="glass-card p-8 rounded-3xl border border-[#22314D] space-y-8 bg-gradient-to-b from-[#030712] to-[#070b14]">
            ...
            <div className="border-b border-[#22314D] pb-6 space-y-2">
              <h2 className="text-2xl font-black text-white flex items-center gap-2.5">
                <FileText className="h-6 w-6 text-blue-500" /> ROOTMASTEROS ENGINEERING MANUAL
              </h2>
              <p className="text-xs text-slate-400 leading-relaxed">
                Official architectural specification sheet details. Coordinates extraction, base bootstrapping, chroot setups, and bootloaders.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-[#020617] p-5 rounded-2xl border border-[#22314D] font-mono text-[11px] leading-relaxed text-slate-300">
                <span className="text-slate-500 font-sans block mb-2 font-bold">// Compressed hybrid ISO image structure:</span>
                RootMasterOS.iso<br />
                ├── boot/                <span className="text-slate-500">← Bootloader components</span><br />
                │   ├── vmlinuz          <span className="text-slate-500">← Linux Kernel v6.8 image</span><br />
                │   ├── initrd           <span className="text-slate-500">← RAMDisk load state</span><br />
                │   └── grub/            <span className="text-slate-500">← GRUB environment configs</span><br />
                │       └── grub.cfg     <span className="text-slate-500">← Bootloader display list menu</span><br />
                ├── filesystem.squashfs  <span className="text-slate-500">← Ultra-compressed read-only rootfs (Ubuntu base + UI + apps)</span><br />
                └── EFI/                 <span className="text-slate-500">← UEFI secure startup modules</span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Compile on any baremetal x86_64 Ubuntu machine with this exact sequence:
              </p>
              ...
              <div className="space-y-4 font-mono text-[11px]">
                ...
                <div className="p-4 rounded-xl border border-[#22314D] bg-[#020617] space-y-2">
                  <div className="flex justify-between items-center text-white font-sans text-xs">
                    <strong className="text-blue-400">Step A: Install Compile Toolchain</strong>
                    <button 
                      onClick={() => handleCopyToClipboard("sudo apt update && sudo apt install -y debootstrap squashfs-tools xorriso grub-pc-bin mtools nodejs npm", "Apt Command")}
                ...
                <div className="p-4 rounded-xl border border-[#22314D] bg-[#020617] space-y-2">
                  <div className="flex justify-between items-center text-white font-sans text-xs">
                    <strong className="text-blue-400">Step B: Debootstrap Ubuntu jammy</strong>
                    <button 
                      onClick={() => handleCopyToClipboard("sudo debootstrap jammy rootfs http://archive.ubuntu.com/ubuntu/", "Debootstrap Command")}
                      className="text-slate-400 hover:text-white"
                    >
                      <Copy className="h-4 w-4" />
                    </button>
                ...
                <div className="p-4 rounded-xl border border-[#22314D] bg-[#020617] space-y-3 text-slate-300">
                  <div className="flex justify-between items-center text-white font-sans text-xs">
                    <strong className="text-blue-400">Step C: Configure Startup `/usr/bin/start-os`</strong>
                    <button 
                      onClick={() => handleCopyToClipboard("#!/bin/bash\ncd /opt/rootmaster\nnode backend/server.js &\nnpm --prefix frontend start", "Startup Script")}
                      className="text-slate-400 hover:text-white"
                    >
                      <Copy className="h-4 w-4" />
                    </button>
                  </div>
                  <span className="text-[10px] text-slate-500 font-sans block">// Injected into rootfs/usr/bin/start-os:</span>
...
npm --prefix frontend start`}
                  ...
                  <span className="text-[10px] text-slate-500 font-sans block">// Grant executions permission:</span>
                  <code className="text-purple-400 block">
                ...
                <div className="p-4 rounded-xl border border-[#22314D] bg-[#020617] space-y-3">
                  <div className="flex justify-between items-center text-white font-sans text-xs">
                    <strong className="text-blue-400">Step D: SquashFS Compression & ISO Output</strong>
                    <button 
                      onClick={() => handleCopyToClipboard("sudo mksquashfs rootfs iso/filesystem.squashfs -e boot\ngrub-mkrescue -o RootMasterOS.iso iso/", "Compile command")}
                      className="text-slate-400 hover:text-white"
                    >
                      <Copy className="h-4 w-4" />
                    </button>
                  ...
                  <code className="text-purple-400 block">
                    sudo mksquashfs rootfs iso/filesystem.squashfs -e boot<br />
            ...
            <div className="space-y-4">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Controls the initial boot options when loading the ISO inside any Virtual machine:
              </p>

              <div className="p-5 rounded-2xl border border-[#22314D] bg-[#020617] font-mono text-[11px] text-purple-400 space-y-1">
                <span>set timeout=5</span><br />
                <span>menuentry "RootMasterOS Bootable Distro (Micki Hart Admin System)" {"{"}</span><br />
                <span className="pl-4">linux /boot/vmlinuz boot=live quiet splash loglevel=3</span><br />
                <span className="pl-4">initrd /boot/initrd</span><br />
                <span>{"}"}</span>
              </div>
            </div>
          ...
          <div className="bg-[#111827] border border-[#22314D] rounded-3xl max-w-4xl w-full p-6 space-y-6 shadow-2xl relative animate-slideIn text-left">
            <div className="flex items-center justify-between border-b border-[#22314D] pb-4">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="text-base font-bold text-white">RootMaster OS Master Project Blueprint Spec Export</h3>
              </div>
              <button 
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              The following structured system specifications, bootloader parameters, database relationships, and 100+ Master Backlog tasks are ready for deployment or project integration.
            </p>
            ...
            <div className="bg-[#0B0F19] border border-[#22314D] rounded-xl p-4 max-h-[380px] overflow-y-auto font-mono text-[10px] text-purple-400 space-y-5 scrollbar-thin selection:bg-[#2F58CD]/30 selection:text-white">
              <div>
                <span className="text-white font-bold">// =========================================================================</span><br />
                <span className="text-white font-bold">// ROOTMASTER LAB PROJECT SPECIFICATION & BACKLOG BLUEPRINT</span><br />
                <span className="text-white font-bold">// Target Platform: Samsung Galaxy S25 Ultra (SM-S938U Verizon Baseline)</span><br />
                <span className="text-white font-bold">// Bootable OS Base: Ubuntu Minimal ISO (debootstrap Linux Kernel v6.8)</span><br />
                <span className="text-white font-bold">// =========================================================================</span><br />
              </div>

              <div>
                <span className="text-blue-400 font-bold"># 1. LIVE OS COMPILING ARCHITECTURE SCHEMA</span><br />
                <span>• ISO NAME: RootMasterOS.iso</span><br />
                <span>• GRUB MENU ENTRY: "RootMasterOS Bootable Distro"</span><br />
                <span>• STARTUP ACTION: Auto Login to XFCE -> Launch `/usr/bin/start-os` script</span><br />
                <span>• SERVICES RUNNING:</span><br />
                <span>    - Node.js backend/server.js (port 5000)</span><br />
                <span>    - React/Next.js frontend dev-server (port 3000)</span><br />
                <span>    - Android Magisk dynamic bindhost mounting loops</span><br />
              </div>

              <div>
                <span className="text-blue-400 font-bold"># 2. SUGGESTED DATABASE SCHEMA REPRESENTATIONS</span><br />
                <span className="text-slate-500 font-sans">// Entities represent PostgreSQL relationships backed by Redis caching:</span><br />
                <span>• Users (id UUID PRIMARY KEY, email VARCHAR UNIQUE, password_hash VARCHAR, clearance_level INT)</span><br />
                <span>• Devices (id UUID, serial_number VARCHAR, model_code VARCHAR, trust_score INT, status VARCHAR)</span><br />
                <span>• FirmwareRecords (id UUID, build_number VARCHAR, system_img_sha256 VARCHAR, patch_date DATE)</span><br />
                <span>• BacklogTasks (id VARCHAR PRIMARY KEY, module_name VARCHAR, title VARCHAR, priority VARCHAR, status VARCHAR)</span><br />
                <span>• AuditLogs (id UUID, operator_id UUID, timestamp TIMESTAMP, action VARCHAR, signature VARCHAR)</span><br />
              </div>

              <div>
                <span className="text-blue-400 font-bold"># 3. ROOTMASTER BACKLOG CHECKLIST PROGRESS DATA ({completedCount} of {totalCount} completed)</span><br />
                {tasks.map(t => (
                  <div key={t.id} className="pl-4">
                    <span>{`[${t.status === 'Completed' ? 'x' : ' '}] ${t.id} (${t.priority}): ${t.title}`}</span><br />
                    <span className="text-slate-500 pl-8">{`↳ ${t.desc}`}</span><br />
                  </div>
                ))}
              </div>
            </div>
